...
// Provides an implementation of stack_cue_list_content_widget_get_type
G_DEFINE_TYPE(StackCueListContentWidget, stack_cue_list_content_widget, GTK_TYPE_WIDGET)
...
GtkWidget *stack_cue_list_content_widget_new()
{
	...
	GtkWidget *widget = GTK_WIDGET(g_object_new(stack_cue_list_content_widget_get_type(), NULL, NULL));
	StackCueListContentWidget *sclw = STACK_CUE_LIST_CONTENT_WIDGET(widget);
	...
	stack_cue_list_content_widget_set_cue_list(sclw, NULL);
...
static int32_t stack_cue_list_content_widget_get_visible_area(StackCueListContentWidget *sclw)
{
...
static int32_t stack_cue_list_content_widget_get_scroll_offset(StackCueListContentWidget *sclw)
{
	GtkWidget *parent = gtk_widget_get_parent(GTK_WIDGET(sclw));
	if (parent == NULL)
	{
...
static void stack_cue_list_content_widget_set_scroll_offset(StackCueListContentWidget *sclw, int32_t scroll_offset)
{
	GtkWidget *parent = gtk_widget_get_parent(GTK_WIDGET(sclw));
	GtkAdjustment *adjustment = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(parent));
...
static gboolean stack_cue_list_content_widget_idle_redraw(gpointer user_data)
{
	// This is here to prevent redraws whilst we're being destroyed
	if (GTK_IS_WIDGET(user_data))
	{
		if (!STACK_CUE_LIST_CONTENT_WIDGET(user_data)->redraw_pending)
		{
			gtk_widget_queue_draw(GTK_WIDGET(user_data));
			STACK_CUE_LIST_CONTENT_WIDGET(user_data)->redraw_pending = true;
		}
	}
...
void stack_cue_list_content_widget_set_row_height(StackCueListContentWidget *sclw, guint row_height)
{
	...
	gdk_threads_add_idle(stack_cue_list_content_widget_idle_redraw, sclw);
}
...
StackCue *stack_cue_list_content_widget_get_cue_at_index(StackCueListContentWidget *sclw, uint32_t search_index, StackCue **placeholder_for)
{
	...
	for (auto citer = sclw->cue_list->cues->recursive_begin(); citer != sclw->cue_list->cues->recursive_end();)
	{
		...
		if (citer.is_child() && !stack_cue_list_content_widget_is_cue_expanded(sclw, cue->parent_cue->uid))
		{
		...
		if (cue->can_have_children && stack_cue_list_content_widget_is_cue_expanded(sclw, cue->uid) && stack_cue_get_children(cue)->size() == 0)
		{
			++current_cue_index;
			if (current_cue_index == search_index)
			{
...
int32_t stack_cue_list_content_widget_get_visible_count(StackCueListContentWidget *sclw)
{
	if (sclw->cue_list == NULL)
	{
		return -1;
	...
	for (auto cue : *sclw->cue_list->cues)
	{
		if (cue->can_have_children)
		{
			...
			if (stack_cue_list_content_widget_is_cue_expanded(sclw, cue->uid) && children != NULL)
			{
				if (children->size() == 0)
				{
...
void stack_cue_list_content_widget_update_height(StackCueListContentWidget *sclw)
{
	...
	int32_t cues_height = sclw->row_height * stack_cue_list_content_widget_get_visible_count(sclw);
...
int32_t stack_cue_list_content_widget_get_list_height(StackCueListContentWidget *sclw)
{
	return stack_cue_list_content_widget_get_visible_count(sclw) * sclw->row_height;
}

int32_t stack_cue_list_content_widget_get_cue_y(StackCueListContentWidget *sclw, cue_uid_t cue_uid)
{
	if (sclw->cue_list == NULL)
	{
		return -1;
	...
	for (auto citer = sclw->cue_list->cues->recursive_begin(); citer != sclw->cue_list->cues->recursive_end(); )
	{
		...
		if (citer.is_child())
		{
			if (!stack_cue_list_content_widget_is_cue_expanded(sclw, cue->parent_cue->uid))
			{
...
bool stack_cue_list_content_widget_ensure_cue_visible(StackCueListContentWidget *sclw, cue_uid_t cue)
{
	bool redraw = false;
	...
	int32_t cue_y = stack_cue_list_content_widget_get_cue_y(sclw, cue);
	if (cue_y == -1)
	{
	...
	guint height = stack_cue_list_content_widget_get_visible_area(sclw);

	bool off_top = cue_y - stack_cue_list_content_widget_get_scroll_offset(sclw) < 0;
	bool off_bottom = cue_y + sclw->row_height - stack_cue_list_content_widget_get_scroll_offset(sclw) > height;

	if (off_top)
	{
		stack_cue_list_content_widget_set_scroll_offset(sclw, cue_y);
		redraw = true;
	}
	else if (off_bottom)
	{
		...
		{
			stack_cue_list_content_widget_set_scroll_offset(sclw, cue_y - height + sclw->row_height);
			redraw = true;
		}
	}
...
bool stack_cue_list_content_widget_is_cue_visible(StackCueListContentWidget *sclw, cue_uid_t cue)
{
	if (sclw->cue_list == NULL || cue == STACK_CUE_UID_NONE)
	{
		return false;
	...
	if ((cue_y + sclw->row_height - stack_cue_list_content_widget_get_scroll_offset(sclw) < 0) || (cue_y - sclw->scroll_offset > (int32_t)height))
	{
		return false;
	}
...
int32_t stack_cue_list_content_widget_get_index_at_point(StackCueListContentWidget *sclw, int32_t x, int32_t y)
{
...
cue_uid_t stack_cue_list_content_widget_get_cue_at_point(StackCueListContentWidget *sclw, int32_t x, int32_t y)
{
	int32_t row_y = 0;
...
static gboolean stack_cue_list_content_widget_idle_update_cue(gpointer user_data)
{
	SCLWUpdateCue *uc = (SCLWUpdateCue*)user_data;
	StackCue *cue = stack_cue_get_by_uid(uc->cue_uid);
	stack_cue_list_content_widget_update_row(uc->sclw, cue, NULL, -1, uc->fields);
...
void stack_cue_list_content_widget_update_cue(StackCueListContentWidget *sclw, cue_uid_t cue_uid, int32_t fields)
{
	if (stack_cue_list_content_widget_is_cue_visible(sclw, cue_uid))
	{
		...
		uc->cue_uid = cue_uid;
		uc->fields = fields;
		gdk_threads_add_idle(stack_cue_list_content_widget_idle_update_cue, uc);
	}
}
...
static void stack_cue_list_content_widget_render_time(StackCueListContentWidget *sclw, cairo_t *cr, double x, double y, double width, const char *text, double pct, GtkStyleContext *style_context)
{
	if (pct > 0.0)
	{
	...
	// Render text on top
	stack_cue_list_content_widget_render_text(sclw, cr, x, y, width, sclw->row_height, text, true, false, style_context);
}
...
static void stack_cue_list_content_widget_render_placeholder(StackCueListContentWidget *sclw, SCLWColumnGeometry *geom, double row_y)
{
	if (sclw == NULL)
	{
		return;
	...
	{
		geom = &local_geom;
		stack_cue_list_content_widget_get_geometry(sclw, geom);
	}
	...
	stack_cue_list_content_widget_render_text(sclw, sclw->list_cr, geom->name_x + (sclw->row_height * 2 - 6), row_y, geom->name_width - (sclw->row_height * 2 - 6), sclw->row_height, "Drag-and-drop cues into this cue", false, true, style_context);
...
static bool stack_cue_list_content_widget_update_row(StackCueListContentWidget *sclw, StackCue *cue, SCLWColumnGeometry *geom, double row_y, const int32_t fields)
{
	...
	if (cue->parent_cue != NULL && !stack_cue_list_content_widget_is_cue_expanded(sclw, cue->parent_cue->uid))
	{
		return false;
	}
	...
	if (row_y < 0)
	{
		int32_t calc_row_y = stack_cue_list_content_widget_get_cue_y(sclw, cue->uid);
		if (calc_row_y == -1)
		{
			return false;
		}
		row_y = calc_row_y - stack_cue_list_content_widget_get_scroll_offset(sclw);
	}
	...
	if (fields == 0 || fields == 2)
	{
		...
		if (cue->id != 0)
		{
			...
			stack_cue_list_content_widget_render_text(sclw, sclw->list_cr, geom->cue_x, row_y, sclw->cue_width, sclw->row_height, buffer, true, false, style_context);
		}
	}
	...
	if ((fields == 0 || fields == 5) && sclw->scriptref_width > 0)
	{
		...
		{
			stack_cue_list_content_widget_render_text(sclw, sclw->list_cr, geom->scriptref_x, row_y, sclw->scriptref_width, sclw->row_height, scriptref_text, true, false, style_context);
		}
	}
	...
	if (fields == 0 || fields == 3)
	{
		...
		stack_cue_list_content_widget_render_text(sclw, sclw->list_cr, geom->name_x + text_x_offset, row_y, geom->name_width - text_x_offset, sclw->row_height, stack_cue_get_rendered_name(cue), false, false, style_context);
	}
	...
	if (fields == 0 || fields == 4)
	{
		...
		stack_cue_list_content_widget_render_time(sclw, sclw->list_cr, geom->pre_x, row_y, sclw->pre_width, pre_buffer, pre_pct, style_context);
		stack_cue_list_content_widget_render_time(sclw, sclw->list_cr, geom->action_x, row_y, sclw->action_width, action_buffer, action_pct, style_context);
		stack_cue_list_content_widget_render_time(sclw, sclw->list_cr, geom->post_x, row_y, sclw->post_width, post_buffer, post_pct, style_context);
	}
...
void stack_cue_list_content_widget_update_list_cache(StackCueListContentWidget *sclw, guint width, guint height)
{
	...
	if (height == 0)
	{
		height = stack_cue_list_content_widget_get_visible_area(sclw);
	}
	...
	size_t count = 0;
	//double row_y = (double)-(stack_cue_list_content_widget_get_scroll_offset(sclw));
	double row_y = -fmod(stack_cue_list_content_widget_get_scroll_offset(sclw), sclw->row_height);
	...
	for (auto citer = sclw->cue_list->cues->recursive_begin(); citer != sclw->cue_list->cues->recursive_end(); )
	{
		...
		if (found_first)
		{
			bool rendered = stack_cue_list_content_widget_update_row(sclw, cue, &geom, row_y, 0);
			if (!rendered)
			{
				break;
			}
			if (cue->can_have_children && stack_cue_list_content_widget_is_cue_expanded(sclw, cue->uid) && stack_cue_get_children(cue)->size() == 0)
			{
				...
				stack_cue_list_content_widget_render_placeholder(sclw, &geom, row_y);
			}
			row_y += (double)sclw->row_height;
		}

		if (!found_first && cue->can_have_children && stack_cue_list_content_widget_is_cue_expanded(sclw, cue->uid) && stack_cue_get_children(cue)->size() == 0)
		{
	...
	sclw->rendered_scroll_offset = stack_cue_list_content_widget_get_scroll_offset(sclw);

	// Unlock the cue list
	stack_cue_list_unlock(sclw->cue_list);
}

static gboolean stack_cue_list_content_widget_draw(GtkWidget *widget, cairo_t *cr)
{
	StackCueListContentWidget *sclw = STACK_CUE_LIST_CONTENT_WIDGET(widget);
	sclw->redraw_pending = false;
	...
	if (sclw->list_surface == NULL || sclw->list_cache_width != width || sclw->list_cache_height != height || sclw->rendered_scroll_offset != stack_cue_list_content_widget_get_scroll_offset(sclw))
	{
		stack_cue_list_content_widget_update_list_cache(sclw, width, height);
	}

	// Render the list
	cairo_set_source_surface(cr, sclw->list_surface, 0, stack_cue_list_content_widget_get_scroll_offset(sclw));
	cairo_rectangle(cr, 0.0, stack_cue_list_content_widget_get_scroll_offset(sclw), sclw->list_cache_width, sclw->list_cache_height);
	cairo_fill(cr);
...
static void stack_cue_list_content_widget_reset_selection(StackCueListContentWidget *sclw)
{
	bool changed = false;
	...
	for (auto iter : sclw->cue_flags)
	{
		if (iter.second & SCLW_FLAG_SELECTED && iter.first != sclw->primary_selection)
		{
			...
			// Redraw
			stack_cue_list_content_widget_update_cue(sclw, iter.first, 0);
		}
	}
	...
	if (!(sclw->cue_flags[sclw->primary_selection] & SCLW_FLAG_SELECTED))
	{
		...
		stack_cue_list_content_widget_update_cue(sclw, sclw->primary_selection, 0);
	}
...
void stack_cue_list_content_widget_select_single_cue(StackCueListContentWidget *sclw, cue_uid_t new_uid)
{
	...
	stack_cue_list_content_widget_reset_selection(sclw);
}
...
void stack_cue_list_content_widget_set_primary_selection(StackCueListContentWidget *sclw, cue_uid_t new_uid)
{
	if (new_uid != sclw->primary_selection)
	{
		...
		{
			// Expand the parent
			stack_cue_list_content_widget_toggle_expansion(sclw, new_cue->parent_cue->uid);
		}
		else
		{
			// Redraw both rows
			stack_cue_list_content_widget_update_cue(sclw, old_uid, 0);
			stack_cue_list_content_widget_update_cue(sclw, new_uid, 0);
		}

		g_signal_emit(G_OBJECT(sclw), signal_primary_selection_changed, 0, new_uid);

		// Redraw
		if (!stack_cue_list_content_widget_ensure_cue_visible(sclw, new_uid))
		{
			gdk_threads_add_idle(stack_cue_list_content_widget_idle_redraw, sclw);
		}
	}
...
void stack_cue_list_content_widget_remove_from_selection(StackCueListContentWidget *sclw, cue_uid_t new_uid)
{
	if (stack_cue_list_content_widget_is_cue_selected(sclw, new_uid))
	{
		stack_cue_list_content_widget_toggle_selection(sclw, new_uid);
		stack_cue_list_content_widget_update_cue(sclw, new_uid, 0);
...
void stack_cue_list_content_widget_toggle_flag(StackCueListContentWidget *sclw, cue_uid_t new_uid, uint32_t flag)
{
	auto iter = sclw->cue_flags.find(new_uid);
	if (iter != sclw->cue_flags.end())
	{
...
{
	stack_cue_list_content_widget_toggle_flag(sclw, new_uid, SCLW_FLAG_SELECTED);
	stack_cue_list_content_widget_update_cue(sclw, new_uid, 0);
...
{
	stack_cue_list_content_widget_toggle_flag(sclw, new_uid, SCLW_FLAG_EXPANDED);
...
bool stack_cue_list_content_widget_has_flag(StackCueListContentWidget *sclw, cue_uid_t uid, uint32_t flag)
{
...
{
	return stack_cue_list_content_widget_has_flag(sclw, uid, SCLW_FLAG_EXPANDED);
}

bool stack_cue_list_content_widget_is_cue_selected(StackCueListContentWidget *sclw, cue_uid_t uid)
{
	return stack_cue_list_content_widget_has_flag(sclw, uid, SCLW_FLAG_SELECTED);
}

static void stack_cue_list_content_widget_motion(GtkWidget *widget, GdkEventMotion *event, gpointer user_data)
{
	StackCueListContentWidget *sclw = STACK_CUE_LIST_CONTENT_WIDGET(widget);
	...
	if (sclw->dragging == 2)
	{
		...
		int32_t new_index = stack_cue_list_content_widget_get_index_at_point(sclw, (int32_t)event->x, (sclw->row_height / 2) + (int32_t)event->y);
		int32_t visible_count = stack_cue_list_content_widget_get_visible_count(sclw);
		if (new_index < 0)
		{
			new_index = 0;
		}
...
static void stack_cue_list_content_widget_button(GtkWidget *widget, GdkEventButton *event, gpointer user_data)
{
	StackCueListContentWidget *sclw = STACK_CUE_LIST_CONTENT_WIDGET(widget);
	...
	if (event->type == GDK_BUTTON_PRESS)
	{
		...
		cue_uid_t clicked_cue = stack_cue_list_content_widget_get_cue_at_point(sclw, event->x, event->y);
		if (clicked_cue != STACK_CUE_UID_NONE)
		{
			...
			if (event->state & GDK_SHIFT_MASK)
			{
				...
				while (!done)
				{
					...
					else
					{
						...
						if (!iter.is_child() || (iter.is_child() & stack_cue_list_content_widget_is_cue_expanded(sclw, iter_cue->parent_cue->uid)))
						{
							stack_cue_list_content_widget_add_to_selection(sclw, iter_cue->uid);
						}
			...
			{
				stack_cue_list_content_widget_toggle_selection(sclw, clicked_cue);
			}

			// If the clicked cue is already selected, don't change the selection immediately,
			// as we could be initiating a drag
			if (!stack_cue_list_content_widget_is_cue_selected(sclw, clicked_cue))
			{
				// Set the new primary row
				stack_cue_list_content_widget_set_primary_selection(sclw, clicked_cue);
			...
			{
				stack_cue_list_content_widget_toggle_expansion(sclw, clicked_cue);
			}
		}
		...
		sclw->drag_index = stack_cue_list_content_widget_get_index_at_point(sclw, event->x, event->y);
	}
	else if (event->type == GDK_BUTTON_RELEASE)
	{
		...
		if (sclw->dragging == 2)
		{
			...
			if (moved)
			{
				...
				if (dest_cue == NULL)
				{
					...
					{
						stack_log("stack_cue_list_content_widget_button(): No cue at index %d\n", new_index);
						sclw->dragging = 0;
						return;
					}
				}
				...
				// expanded cue
				if (dest_cue->parent_cue != NULL || (!before && stack_cue_list_content_widget_is_cue_expanded(sclw, dest_cue->uid)))
				{
					dest_is_child = true;
				}
				...
				for (auto iter = sclw->cue_list->cues->recursive_begin(); iter != sclw->cue_list->cues->recursive_end(); ++iter)
				{
					...
					if (stack_cue_list_content_widget_is_cue_selected(sclw, moving_cue->uid))
					{
				...
				else
				{
					...
					for (StackCue *moving_cue : cues_to_move)
					{
						...
						// Move the cue
#ifndef NDEBUG
						stack_log("stack_cue_list_content_widget_button(): Moving cue 0x%016llx %s cue 0x%016llx (%d) - child: %s\n", moving_cue, before ? "before" : "after", dest_cue->uid, dest_cue->id, dest_is_child ? "true" : "false");
#endif
		...
		else
		{
			...
			cue_uid_t clicked_cue = stack_cue_list_content_widget_get_cue_at_point(sclw, event->x, event->y);
			if (clicked_cue != STACK_CUE_UID_NONE && stack_cue_list_content_widget_is_cue_selected(sclw, clicked_cue) && !(event->state & (GDK_CONTROL_MASK | GDK_SHIFT_MASK)))
			{
				// Set the new primary row
				stack_cue_list_content_widget_set_primary_selection(sclw, clicked_cue);
...
gboolean stack_cue_list_content_widget_key_press(GtkWidget *widget, GdkEventKey *event, gpointer user_data)
{
	...
	if (event->keyval == GDK_KEY_Up)
	{
		...
		else
		{
			...
			if (iter != sclw->cue_list->cues->recursive_end())
			{
				...
				if (iter.is_child() && !stack_cue_list_content_widget_is_cue_expanded(sclw, (*iter)->parent_cue->uid))
				{
	...
	else if (event->keyval == GDK_KEY_Left)
	{
		if (sclw->primary_selection != STACK_CUE_UID_NONE)
		{
			...
			if (cue != NULL && cue->parent_cue != NULL)
			{
				...
				stack_cue_list_content_widget_select_single_cue(sclw, cue->parent_cue->uid);
			}
			else
			{
				if (stack_cue_list_content_widget_is_cue_expanded(sclw, sclw->primary_selection))
				{
					stack_cue_list_content_widget_toggle_expansion(sclw, sclw->primary_selection);
				}
			}
		}
	}
	...
	else if (event->keyval == GDK_KEY_Page_Up || event->keyval == GDK_KEY_KP_Page_Up)
	{
		...
		else
		{
			...
			const guint list_display_height = stack_cue_list_content_widget_get_visible_area(sclw);
	...
	if (select_new_cue)
	{
		if (event->state & GDK_SHIFT_MASK)
		{
			stack_cue_list_content_widget_add_to_selection(sclw, new_cue_uid);
			stack_cue_list_content_widget_set_primary_selection(sclw, new_cue_uid);
		}
		else
		{
...
static gboolean stack_cue_list_content_widget_focus(GtkWidget *widget, GdkEventFocus *event)
{
	// Redraw
	stack_cue_list_content_widget_update_list_cache(STACK_CUE_LIST_CONTENT_WIDGET(widget), 0, 0);
	gdk_threads_add_idle(stack_cue_list_content_widget_idle_redraw, STACK_CUE_LIST_CONTENT_WIDGET(widget));

	return true;
}

static void stack_cue_list_content_widget_init(StackCueListContentWidget *sclw)
{
...
static void stack_cue_list_content_widget_realize(GtkWidget *widget)
{
...
static void stack_cue_list_content_widget_unrealize(GtkWidget *widget)
{
	StackCueListContentWidget *sclw = STACK_CUE_LIST_CONTENT_WIDGET(widget);
...
static void stack_cue_list_content_widget_map(GtkWidget *widget)
{
	// Chain up
	GTK_WIDGET_CLASS(stack_cue_list_content_widget_parent_class)->map(widget);

	gdk_window_show(STACK_CUE_LIST_CONTENT_WIDGET(widget)->window);

	stack_cue_list_content_widget_ensure_cue_visible(STACK_CUE_LIST_CONTENT_WIDGET(widget), STACK_CUE_LIST_CONTENT_WIDGET(widget)->primary_selection);
}

static void stack_cue_list_content_widget_unmap(GtkWidget *widget)
{
	gdk_window_hide(STACK_CUE_LIST_CONTENT_WIDGET(widget)->window);

	// Chain up
	GTK_WIDGET_CLASS(stack_cue_list_content_widget_parent_class)->unmap(widget);
}

static void stack_cue_list_content_widget_finalize(GObject *obj)
{
	StackCueListContentWidget *sclw = STACK_CUE_LIST_CONTENT_WIDGET(obj);
	...
	// Chain up
	G_OBJECT_CLASS(stack_cue_list_content_widget_parent_class)->finalize(obj);
}

static gboolean stack_cue_list_content_widget_query_tooltip(GtkWidget* widget, gint x, gint y, gboolean keyboard_tooltip, GtkTooltip* tooltip)
{
	if (keyboard_tooltip)
	{
		return false;
	...
	cue_uid_t cue_uid = stack_cue_list_content_widget_get_cue_at_point(STACK_CUE_LIST_CONTENT_WIDGET(widget), x, y);
...
static void stack_cue_list_content_widget_class_init(StackCueListContentWidgetClass *cls)
{
	// Things we need to override at the class level
	GObjectClass *object_cls = G_OBJECT_CLASS(cls);
	object_cls->finalize = stack_cue_list_content_widget_finalize;

	// Things we need to override at the widget level
	GtkWidgetClass *widget_cls = GTK_WIDGET_CLASS(cls);
	widget_cls->draw = stack_cue_list_content_widget_draw;
	widget_cls->realize = stack_cue_list_content_widget_realize;
	widget_cls->unrealize = stack_cue_list_content_widget_unrealize;
	widget_cls->map = stack_cue_list_content_widget_map;
	widget_cls->unmap = stack_cue_list_content_widget_unmap;
	widget_cls->query_tooltip = stack_cue_list_content_widget_query_tooltip;

	// Setup signals for selection changes
	signal_primary_selection_changed = g_signal_new("primary-selection-changed", stack_cue_list_content_widget_get_type(), G_SIGNAL_RUN_FIRST, 0, NULL, NULL, NULL, G_TYPE_NONE, 1, G_TYPE_INT64);
	signal_selection_changed = g_signal_new("selection-changed", stack_cue_list_content_widget_get_type(), G_SIGNAL_RUN_FIRST, 0, NULL, NULL, NULL, G_TYPE_NONE, 0);
}
